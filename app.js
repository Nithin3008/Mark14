...
function profit(ip, sp, q) {
    ...
    msg.innerText = "Profit you received " + profit1 + "/-, profit percentage " + profitPer.toFixed(2) + "%"
}
function loss(ip, sp, q) {
    ...
    msg.innerText = "Loss occured " + loss1 + "/-, loss percentage " + lossPer.toFixed(2) + "%"
...
function calculateProfitAndLoss() {
    ...
    if (ip === 0 || q === 0 || sp === 0) {
        msg.innerText = "Enter proper values"
    }
    else {
        if (ip > sp) {
            loss(ip, sp, q)
